...
export function parseUrlState(): ScrubberUrlState {
  ...
  const scrubSpeedSlow = params.get("scrubSpeedSlow");
  ...
  return {
    ...
    scrubSpeedSlow: scrubSpeedSlow != null ? Number(scrubSpeedSlow) : undefined,
...
export function buildSearchParams(state: {
  ...
  if (state.scrubSpeedSlow != null && Number.isFinite(state.scrubSpeedSlow)) params.set("scrubSpeedSlow", String(state.scrubSpeedSlow));
...
export function applyUrlStateToSettings(
...
): Partial<{ videoId: string; seekTime: number; speed: number; slowMoSpeed: number; scrubSpeedSlow: number; scrubSpeedFast: number }> {
  const out: Partial<{ videoId: string; seekTime: number; speed: number; slowMoSpeed: number; scrubSpeedSlow: number; scrubSpeedFast: number }> = {};
  ...
  if (urlState.scrubSpeedSlow != null && Number.isFinite(urlState.scrubSpeedSlow)) out.scrubSpeedSlow = urlState.scrubSpeedSlow;
